...
function showUpdateForm(expenseId) {
    ...
    document.getElementById("expense-id-update").value = expenseId;
    ...
    document.getElementById("store-update").value = cells[3].textContent;
    document.getElementById("purchase-date-update").value = cells[4].textContent;

    // Show the update modal form
    document.getElementById("update-expense-form-container").style.display = "block";
}

// Function to hide the update modal form
function hideUpdateModal() {
    document.getElementById("update-expense-form-container").style.display = "none";
}

// Function to handle update button click
function updateExpense() {
// Get the form data
var formData = new FormData(document.getElementById("update-expense-form"));
...
document.getElementById("update-cancel-button").addEventListener("click", function() {
    ...
    if (product === "" || quantity === "" || price === "" || store === "" || purchaseDate === "") {
...
var options = {
    filename: 'expenses.pdf', // Set the filename
...
fetch("get_expenses_by_date.php", {
    method: "POST",
    headers: {
...
.then(data => {
    ...
    document.querySelector(".expenses-table tbody").innerHTML = data;

    // Store the filtered data in localStorage
    localStorage.setItem('filteredExpenses', data);
})
.catch(error => {
